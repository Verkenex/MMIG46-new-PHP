...
final class AuthController {
    ...
    public function doLogin(): string { Security::verifyCsrf(); $email=trim($_POST['email']??''); $ip=$_SERVER['REMOTE_ADDR']??''; $pdo=DB::pdo();
        $q=$pdo->prepare('SELECT COUNT(*) c FROM login_attempts WHERE ip_address=? AND attempted_at > DATE_SUB(NOW(), INTERVAL 15 MINUTE)'); $q->execute([$ip]); if((int)$q->fetch()['c']>=8){Session::flash('error','Zu viele Loginversuche. Bitte spaeter erneut versuchen.');
        if (($user['role'] ?? '') === 'admin') {
